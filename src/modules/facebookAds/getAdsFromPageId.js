...
export const getAdsFromPageId = async (searchTerm = 'BetterMe') => {
  console.log('Getting Ads For :>> ', searchTerm)
  const { page, browser } = await getBrowser()

  let sessionId = null
  const intercepts = []
  const pageId = 116700222203350
        ...
        try {
          const fetchAdPagination = async (sessionId, pageId, ads = [], forwardCursor = null) => {
            ...
            const req = await fetch(url, {
              ...
              referrer:
                `https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country=US&view_all_page_id=${pageId}&search_type=page&media_type=all`,
            ...
            const flattenedAds = ads.flat()
              return { ads: flattenedAds, totalAds }; // Return early with collected results
            }
